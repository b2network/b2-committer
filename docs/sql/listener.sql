Create Database If Not Exists b2_committer_main Character Set UTF8;
USE b2_committer_main;
...
DROP TABLE IF EXISTS `txs_proposal`;
CREATE TABLE `txs_proposal`
(
    `id`                 bigint       NOT NULL AUTO_INCREMENT,
    `created_at`         datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`         datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `proposal_id`        bigint       NOT NULL COMMENT ' proposal_id',
    `start_timestamp`    bigint       NOT NULL COMMENT ' start_timestamp',
    `end_timestamp`      bigint       NOT NULL COMMENT ' end_timestamp',
    `start_block_number` bigint       NOT NULL COMMENT ' start_block_number',
    `end_block_number`   bigint       NOT NULL COMMENT ' end_block_number',
    `txs_root`           varchar(128) NOT NULL COMMENT ' txs_root',
    `status`             tinyint(8)   NOT NULL COMMENT ' status',
    `ds_type`            varchar(128) NOT NULL COMMENT ' ds_type',
    `ds_tx_hash`         varchar(128) NOT NULL COMMENT ' ds_tx_hash',
    `winner`             varchar(128) NOT NULL COMMENT ' winner',
    `block_list`         varchar(128) NOT NULL COMMENT ' block_list',
    PRIMARY KEY (`id`),
    KEY `op_blob_index` (`id`)
) ENGINE = InnoDB
  AUTO_INCREMENT = 1000000
